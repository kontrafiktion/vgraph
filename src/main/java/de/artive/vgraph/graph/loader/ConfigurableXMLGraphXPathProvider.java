...
import de.artive.vgraph.helper.ResourceLoader;
...
public class ConfigurableXMLGraphXPathProvider implements GraphXPathProvider {
  ...
  /**
   * (mandatory) references all elements that should be loaded as {@link de.artive.vgraph.graph.Node Nodes} into the {@link de.artive.vgraph.graph.Graph}
   */
  public static final String NODES_PATH = "nodes";

  /**
   * (mandatory) XPath that references all elements that should be loaded as {@link de.artive.vgraph.graph.Edge Edges}
   */
   ...
   * @throws IOException
   * @throws de.artive.vgraph.VGraphException
   */
  public ConfigurableXMLGraphXPathProvider(String propertiesResourceName) throws IOException, VGraphException {
    new ResourceLoader() {
      @Override
      protected void slurp(Reader resource) throws IOException {
        ...
        if (error) {
          throw new VGraphException("missing mandatory properties: '" + missingProperties + "'");
        }
      }
