...
import de.artive.vgraph.graph.loader.ConfigurableXMLGraphXPathProvider;
...
public class SimpleXmlSourceLoaderTest {
  ...
  /**
   * Check that an empty model results in {@link de.artive.vgraph.ErrorCode#LOADER_EMPTY_GRAPH}
   *
   * @throws IOException
   */
  @Test
   ...
   * de.artive.vgraph.ErrorCode#XML_NODE_NOT_FOUND}
   *
   * @throws IOException
   */
  @Test
  ...
  private void loadWithExpectedErrorCode(String source, ErrorCode errorCode) {
    ...
    try {
      loader.loadInto(source,
                      XPATH_PROVIDER,
                      graph);
      Assert.fail("expected VGraphException with error code: " + errorCode);
